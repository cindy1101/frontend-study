{
    ...
    // : 컴포넌트가 처리할 데이터 타입을 미리 지정하지 않고, 해당 컴포넌트를 사용하는 시점에서 원하는 데이터 타입 지정 가능
    ...
    // ? 3. 제네릭 기본 문법
    // : 사용할 컴포넌트의 이름 뒤에 꺽쇠괄호(<>) 안에 타입 변수를 넣는 것
    // : 함수나 클래스 등에서 사용할 타입을 명시
    // : 임의의 키워드를 사용하여 명시 
    // : 일반적으로 대문자 알파벳 하나를 사용 (주로 T 라는 이름으로 사용이 된다.) (T: Type)
    // : 해당 타입 변수는 컴포넌트 내에서 실제 데이터 타입의 자리를 대신

    function generic<T>(arg : T): T{
        // T(type)는 타입 변수 (단일 타입 변수)
        // : 해당 타입 변수는 입력 인수의 타입을 T로 지정하고, 함수의 반환 타입을 T로 지정 -> 매개변수 T 타입, 반환값 T 타입 (제네릭 타입을 가져올 수 있다.)
        return arg;
    }

    let stringOutput = generic<string>('안녕하세요');
    // let stringOutput2 = generic<string>(500); TypeError 함수를 호출하는 시점에서 string으로 지정해달라 했는데 다른 타입으로 지정하면 안 된다. 
    ...
    // let pairOutput2 = pair<string, number>(55, '안녕'); -> Type Error
    ...
    // 제네릭 타입 미지정시 인자값으로 타입을 유추할 수 있다.
