{
    ...
    // : active 속성은 기본값으로 true 값을 가짐 (회원이 기본적으로 활성 상태)

    // ? 회원 관리 프로그램
    // : 회원을 추가 (addMember) / 회원을 검색 (findMember) / 회원을 제거 (removeMember) / 활성화된 회원만 필터링 (filterActiveMember)
    ...
    // : 제네릭을 사용하여 정의
    // - Member 클래스 또는 그 자식 클래스의 인스턴스 만을 회원으로 관리
    // : Member 객체의 리스트를 관리
    ...
    class Membership<T extends Member> {
        ...
        // ? 새로운 멤버를 추가하는 메서드를 정의
        // : 회원 리스트에 매개변수를 받은 회원 객체를 추가
        ...
        // : 해당 이름의 회원이 있으면 그 객체를, 없으면 undefined를 반환시켜준다.
        ...
        // * findIndex 메서드
        // : 주어진 함수를 만족하는 첫 번째 요소의 인덱스 값을 반환
        ...
        removeMember(name: string): void {
            ...
            if(index !== -1) {// 해당 이름의 회원이 존재
                ...
                // : 첫 번째 인자는 변경을 시작할 배열의 인덱스, 두 번째 인자는 제거할 요소의 수
