{   
    ...
    class Car {
        ...
        // * 생성자에서 변수 선언과 초기화가 모두 이루어지려면, 반드시! 매개변수에 접근제한자를 표기하여지 한다.
    ...
    class BankAccount {
        ...
        // - accounNumber(계좌번호), balance(잔액) 속성은 외부에서 직접 접근할 수 없는 속성
        // - 생성자 : 계좌번호와 초기 잔액을 매개변수로 받아 객체를 초기화, 초기 잔액은 선택적으로 처리 (기본값 O)
        ...
        // 2. withdraw(amount: number) : 출금액을 받아 잔액을 감소, 잔액보다 많은 금액을 출금할 수 없다.
        // 3. getBalance() : 현재 잔액을 반환 -> 클래스 내부
        // 3. getAccountNumber() : 계좌번호를 반환 -> 클래스 내부
        ...
        withdraw(amount: number) {
            ...
            if(amount > this.balance) {
                console.log('출금액이 잔고를 초가하였습니다! 다시 시도하세요.');
            }else {
                this.balance -= amount;
    ...
    accountA.withdraw(8000); // 입금액 8000, 현재 잔액 7000
    accountA.withdraw(8000); // 출금액이 잔고를 초가하였습니다! 다시 시도하세요.
