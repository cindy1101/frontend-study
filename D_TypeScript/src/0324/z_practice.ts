{
    ...
    type BookType = {
        title: string;
        author: string;
        isbn: string; // 책의 고유번호 (전세계적으로 통용된 번호)
    }
    ...
    class Libraty {
        ...
        public addBook(book: BookType): void {
            ...
            // 배열.push
            console.log(`책 제목이 ${book.title}이(가) 추가되었습니다.`)
        }

        public removeBook(isbn: string):void {
            // 배열의 findIndex 메서드를 사용 (콜백함수)
            // : 책을 순회하며 매개변수의 isbn 번호와 일치할 경우 해당 요소의 인덱스 번호를 찾아옴
            // : 해당 인덱스 번호가 -1인 경우 (해당 isbn 번호가 존재하지 않음) -> 메시지 출력
            // : -1이 아닌 경우 해당 인겟스를 splice로 삭제
        }
