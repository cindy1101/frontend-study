{
    // ! 1. 클래스(Class) 
    // : 객체를 만들 틀

    // ? 클래스 정의
    // : 객체(데이터)를 생성하기 위한 템플릿 또는 설계도
    // : 데이터와 해당 데이터를 조작하는 코드를 함께 캡슐화(묶는다는 개념)

    // ? 클래스 특징
    // : 캡슐화 - 데이터(속성)와 데이터를 처리하는 함수(메서드)를 하나로 묶는 것
    // : 추상화 - 복잡한 세부사항은 정의하지 않으며 필수적인 부분만을 강조하는 것
    // : 상속 - 한 클래스의 속성과 메서드를 다른 클래스가 상속받을 수 있는 것
    // : 다형성 - 하나의 인터페이스를 통해 다른 클래스의 객체(인스턴스)를 다룰 수 있는 것

    // ! 2. 객체(Object)
    // : 틀로 만들어져 실제로 보여지는 것

    // ? 객체 정의
    // : 클래스에 의해서 생성된 인스턴스(instance)
    // : 실제 프로그램에서 사용하는 데이터와 기능의 집합체

    // ? 객체 특징
    // : 상태와 행동 - 객체는 속성(상태)과 메서드(행동)를 갖는 것
    // : 독립성 - 각 객체는 독립적인 성격을 가지며 다른 객체와 상호작용 가능 

    // ! 클래스를 '정의'한다 & 객체를 '생성'한다.

    // ? 클래스 정의
    // : class 키워드를 사용하여 정의
    // : 클래스명은 대문자로 시작한다.

    // * 클래스의 기본 구조
    class ClassName {
        
        // 속성(상태) : (멤버) 변수, 클래스의 상태 저장

        // 메서드(행동) : 함수, 클래스의 동작을 정의

        // * 메서드의 종류 
        // : 인스턴스 메서드 - 일반적인 메서드, 클래스의 인스턴스를 통해 호출, 해당 객체의 속성에 접근이 가능하다
        // : 정적 메서드 - 클래스 자체에 속하는 메서드, 인스턴스 없이 클래스명을 사용하여 호출

        // ? 인스턴스(instance)
        // : 클래스를 통해 생성된 객체
        // : 실제로 생성된 객체이자 '컴퓨터 메모리에 할당된 객체' -> 데이터 처리가 완료되었다.

    }


    // ! 객체 & 클래스

    // * 객체
    // : 서로 많은 데이터를 하나로 묶어서 표현한 것
    // : 현실 세계의 사물이나 개념을 프로그램 내에서 표현한 것

    // (EX) 웹 페이지에서 작성할 '게시글'
    //      게시글 번호, 작성자, 제목, 내용, 작성일자, 수정일자 등 -> 데이터(속성)
    //      게시글 생성, 수정 등 -> 위 데이터(속성)를 처리하는 기능 (메서드, 행동)

    // * 클래스
    // : 객체를 만드는 도구
    // : 클래스를 정의하면 해당 클래스에 속한 객체들이 가져야 할 속성과 메서드를 명시
    // : 메모리 할당 X

    // 붕어빵(객체) & 붕어빵 틀(클래스)
    // : 같은 클래스로 만든 객체라도 각각의 객체는 서로 다른 값을 가짐

    // ? 인스턴스(instance)
    // : 클래스에 의해 생성되어 '메모리에 할당'된 객체
    // : 클래스로부터 객체를 생성하는 과정을 '인스턴스화' 라고 부른다.

    // 붕어빵 틀로 만든 붕어빵은 객체(Object)이며 붕어빵은 붕어빵 틀로 만든 인스턴스(instance)
    // 객체와 인스턴스는 같다 (객체 === 인스턴스)

    // ? 예시
    // '이승아'은 '사람' 클래스의 인스턴스 -> 사람을 만들 때 만드는 방법을 명시해서 (클래스) 사람(객체)을 만든다!
    // 'BNW'는 '자동차' 클래스의 인스턴스
    // 'TS' 는 '프로그래밍 언어' 클래스의 인스턴스
    // '진라면'는 '라면' 클래스의 인스턴스

    // ! 클래스 '정의' / 객체 '생성'
    class Book {

        // ? 속성 
        // : 멤버 변수로 정의한다.
        // : 클래스에 속하는 변수

        title!: string; // 책의 제목 속성 
        author!: string; // 책의 저자 속성 

        // * 클래스를 생성하고 멤버 변수 선언 시 아래의 오류 발생 (객체 생성하면 데이터 넣어 질 거야!)
        // Error - 속성 author은(는) 이니셜라이저가 없고 생성자에 할당되어 있지 않습니다.
        // -> 변수명 오른쪽에 !를 사용하여 컴파일러에게 해당 변수가 값이 무조건 할당되어 있음을 명시적으로 전달
        // 생성자를 작성
        // constructor(){
            // 생성자
        // }

        // ? 메서드
        // : 클래스 내부에서 정의된 함수를 의미한다.
        // : 객체의 행동을 나타냄
        // : 클래스의 인스턴스를 통해 호출

        describe(): string {

            // * this 키워드
            // : 해당 메서드가 쓰이는 클래스의 멤버 변수에 접근
            // : 현재 인스턴스를 가리킴
            // : A와 B라는 책이 있고 A의 타이틀 A, B의 타이틀 B 즉, 결국 같은 Class로 만들어진 책 나중에 부를 때(!, 오류) 각각 인스턴스에서 요소 그 자체를 담는다.

            return `${this.title} is written by ${this.author}`
        }
    }

    // ! 객체 생성
    // : 클래스의 인스턴스는 new 키워드를 사용하여 생성

    // * new 클래스명();
    // : new 키워드를 사용하여 인스턴스를 생성
    // : 생성된 인스턴스를 변수에 할당하여 사용
    // : ()는 클래스의 생성자 함수를 호출
    // : 기본적으로 클래스에는 반드시 하나 이상의 생성자가 존재
    // : 명시하지 않을 경우 매개변수가 없는 빈 생성자가 생략

    let myBook1 = new Book(); 
    let myBook2 = new Book(); 

    // 생성된 인스턴스로 Book 클래스에 정의된 멤버변수와 메서드에 접근
    // : . 연산자를 사용하여 접근

    console.log(myBook1.title); //undefined
    console.log(myBook1.describe()); //undefined is written by undefined
    console.log(myBook2.describe()); //undefined is written by undefined

    // 클래스를 통해 인스턴스 생성 시 
    // : 생성자를 통해 클래스의 멤버 변수를 초기화
    // : 해당 멤버 변수를 초기화하지 않은 경우 undefined로 인식한다.
}