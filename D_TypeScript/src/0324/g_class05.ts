{
    ...
    // * 1. 추상 클래스
    // : 인스턴스화 할 수 없고, 하나 이상의 추상 메서드를 포함할 수 있는 클래스
    // : 공통의 기능을 정의, 상속을 통해서 해당 기능을 구현하도록 강제하는 틀을 제공합니다.
    ...
    abstract class Animal {
        ...
        // 추상클래스 내에서는 반드시 한 개 이상의 추상 메서드가 정의되어야 한다!
        // 그 외에는 다른 일반 메서드와 일반 멤버 변수 정의도 가능하다.

        // 추상 메서드의 정의
        // : 중괄호의 로직 구현부가 존재하지 않는다.
    ...
    // : 메서드와 속성의 시그니처를 정의하는 구조체를 의미한다.
    // : 클래스가 특정 기능을 구현하도록 강제(엔터 치면 {} 생기는), 다형성을 지원 -> 추상 클래스와 같은 개념
    ...
    //   : 상속받는 하위 클래스가 부모 클래스의 특정 종류로 간주
    //   (예) 동물 추상 클래스를 상속받는 개, 고양이 클래스

    // * 2. 인터페이스는 클래스가 '할 수 있는 (has - a)' 관계를 나타냄
    //   : 해당 기능을 할 수 있음에 중점
    //   (예) 비행 가능 인터페이스가 존재한다면 구현하는 '새'와 '비행기' 클래스가 존재한다.
