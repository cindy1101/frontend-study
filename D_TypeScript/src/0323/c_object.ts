{
    ...
    // : 객체'리터럴'은 객체를 정의하고 각 속성에 값을 할당한다.

    console.log(user.name); // 이승아
    console.log(user['height']); // 159
    ...
    greet(seungah); // name과 age가 다 있음 -> Person 이겠네!

    // 구조적 타이핑에 의해 Person으로 취급
    // : hobby 속성을 무시한 채로 Person으로 인식
    greet(dokyung); // name과 age가 다 있어, hobby가 있긴 하지만 -> Person 이네!
    ...
    // : 해당 객체를 생략하는 경우를 대비하여 오류를 발생, 99번
    // : 오류를 방지하기 위해서 조건문을 사용
    ...
    let user1: UserRecord = {
        name: '이승아',
        // UserRecord 타입은 속성명이 문자열 속성값도 반드시 문자열인 다양한 속성을 지정 가능
        // age: 20,
        // isStudent: false
