{
    ...
    console.log(arrowGreet('이승아')); 

    // ! 선택적 매개변수와 기본 매개변수
    
    // * 선택적 매개변수
    // : 함수 호출시 생략 가능, 매개변수를 선택적으로 허용
    // : 변수명 뒤에 ? 작성
    ...
    // name 필수사항 X, greeting은 필수사항이지만 hi라는 기본값이 있다!
    function selevct(name?: string, greeting: string = 'hi'): string {
        if (name){
    ...
    // name 생략되고, greeting의 기본값이 출력 -> name이 없으니까, else 문
    console.log(selevct()); // hi

    // 'hello'의 값이 name의 값으로 지정 -> 의도와 다르게 인사말이 이름으로 지정
    console.log(selevct('hello')); // hi hello

    // 'las'가 name으로, 'hello'가 greeting으로 설정
    console.log(selevct('hello', 'lsa')); // lsa hello

    // ? 선택적 매개변수와 기본 매개변수의 차이
    // : 두가지 모두 함수 호출시 생략 가능
    // - 선택적 매개변수 생략된 경우 undefined로 처리
    // - 기본 매개변수 생략된 경우 지정된 기본값을 사용

    // ? 주의사항
    // 1. 선택적 매개변수는 반드시 필수 매개변수 뒤에 작성
    // 2. 기본 매개변수는 필수 매개변수와 선택적 매개변수 양 쪽에 작성 가능
    // 3. 기본 매개변수가 필수 매개변수 앞에 작성된 경우
    // : 기본 매개변수를 생략하려면 반드시 undefined을 전달

    console.log(selevct(undefined, 'hello'));
    ...
    function sum(...numers: number[]): number {
        ...
        return numers.reduce((a, b) => a + b, 0);
        // a += b
    ...
    // ? Rest 매개변수 주의사항
    // : 항상 마지막에 위치
    // : 타입 명시를 주의해야한다. (묶이는 데이터가 같아야지 된다)


    // ! 함수의 오버로딩
    // : 같은 이름의 함수에 대해 여러 매개변수의 타입과 반환 타입을 정의하는 기능
    ...
    // 기본 문법
    // : 함수 선언부는 여러개가 될 수 있지만, 구현은 하나만 있어야 한다.
