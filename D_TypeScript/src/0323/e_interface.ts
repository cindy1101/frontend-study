{
    ...
    // - 코드의 가독성과 유지 보수성을 향상시키며 다양한 구현체에 동일한 인터페이스를 적용하여 일관성과 재사용성을 제공
    ...
    // 2. 코드 가독성 향상
    // 3. 코드 재사용 촉진
    // 4. 확장 가능성 : 기본 인터페이스를 확장(상속)하여 멤버를 추가하거나 재정의 가능
    ...
    // ? 인터페이스 [interface]
    // : 확장성 (extends 키워드를 사용)
    // : 합치기 (동일한 이름의 인터페이스를 여러 번 선언 - 자동으로 하나의 인터페이스로 처리)
    // : 주로 객체의 모양을 정의하는데 사용
