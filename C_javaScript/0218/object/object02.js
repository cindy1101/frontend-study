...
// : 모든 객체들이 메서드와 속성들을 상속 받기 위한 템플릿(틀)로서 프로토타입 객체를 가짐
// : 정의된 객체를 기반으로 생성된 다른 객체들이 공통적으로 사용할 수 있는 속성과 매서드를 제공 

// ! 생성자 함수를 사용한 객체 생성 (객체의 틀을 정의)
// 생성자 함수를 사용할 경우 코드를 재사용하여 같은 구조의 객체를 여러 개 생성 가능
// : new 키워드를 사용하여 새로운 객체를 생성
// : 관례적으로 함수명의 첫 글자는 대문자로 시작 (일반적 기능을 담은 함수와 구별)
...
} // 템플릿, 붕어빵 틀을 만든다. Person 을 찍어내는 틀을 만든다.

// 이 객체를 만드는 것 new, 팥을 넣을지 슈크림 넣을지... 그럼 팥붕 슈붕
// 생성자 함수를 사용한 객체 생성
let person1 = new Person('승아', '이', 28, 'female', ['exercise', 'develop']);
// >> 생성자 함수로 만들어진 객체 내의 this는 new 키워드를 통해 만들어진 객체 그 자체
...
// : 각 클래스는 하나의 construtor(생성자)만을 가짐

// * 클래스의 메서드
// : 클래스 기반으로 생성된 객체가 공통적으로 가질 수 있는 함수
// : 클래스 내부에서 메서드 정의 시 function 키워드를 사용하지 않는다.

// ? 클래스 생성 방법
// class 키워드를 사용, 클래스명은 대문자로 시작을 권장
