...
//greeting은 processUserInput 함수의 인자로 전달되는 콜백함수
processUserInput(greeting);


// ! 3. 콜백 함수를 사용하는 비동기 요청 예시
// : 웹 API 요청 - 사용자의 데이터를 콜백함수로 전달하여 호출

// fetch : 가서 어떤 것 또는 어떤 이를 다시 가져오는 것이다.
//       : === 데이터를 가져오다

function fetchUserData (userid, callback) {

    // 시간에 대한 비동기 작업을 처리하는 setTimeout() 콜백함수 사용
...
// (EX)
// A 함수가 33번째 줄에서 setTimeout 설정으로 3초 뒤에 실행
// B 함수가 40번째 줄에서 일반 함수로서 호출
// -> 해당 파일 실행 시 A / B 중 먼저 실행되는 코드 (B)


// ! 4. 비동기 프로그래밍으로써 콜백 함수의 단점
// : 콜백함수는 비동기 작업이 중첩될 수록 코드의 인자로 전달되는 코드의 중복이 늘어남

// ? 콜백 지옥 (callback hell)
// : 중첩된 콜백 함수들이 여러 단계에 걸쳐서 깊게 중복되는 문제점
// : 가독성 저하, 디버깅 어려움, 유지관리 어려움
// : 계단식 코드가 순차적으로 이어짐
...
function loginUser(email, password, callback) {
    ...
    // 매개변수로 전달받는 email과 password를, 데이터베이스 정보와 비교
    const user = databaseUsers.find(
        // email, password 은 매개변수
    ...
    if(user) {
        ...
        // : 객체의 구조 분해 할당을 사용 (+ 스프래드 연산자)

        // const { password, name, emil } = user;
...
// 사용자의 역할(rolse)을 기반으로 사용자에게 특정 접근 권한이 있는지 확인하는 함수
function checkAccesss(roles, callback){
    ...
    if(accessGranted) {
        callback(null, true); // 겁근권한 있음
    }else{
        callback(null, false); // 접근권한 없음
    }

}

// 함수 호출 
// : 로그인이 완료되기 전에 첫 번째 익명 함수를 호출
// : 첫 번째 익명함수는 로그인 시 발생하는 오류를 실행
// : 두 번째 익명함수(checkAccesss)는 해당 사용자가 해당 페이지에 접근이 가능한지에 대한 기능을 판단
...
loginUser('example@naver.com', 'password', function (error, user) {
    ...
    // ? 익명 함수 실행 완료 전에 checkAccesss 함수 실행
    checkAccesss(roles, function(error, access){
        if(error){
            return console.error(error);
        }
