// ! 카드 메모리 게임 (카드 매칭 게임)
// : 사용자가 카드를 클릭하면 뒤집고, 같은 색상의 카드를 매칭시키는 게임
...
document.addEventListener('DOMContentLoaded', () =>{
    ...
    // ! HTML 요소를 선택
    // cardContainer 요소의 자식 요소로 생성될 카드를 선택
    const cardContainer = document.getElementById('#card-container');
    const startButton = document.getElementById('#start-button');
    const resetButton = document.getElementById('#reset-button');
    const completedButton = document.getElementById('#ompleted-button');

    // ! 색상 배열을 정의 (6개)
    const colors = ['#cc3131', '#e3ddbb', '#23b923', '#34bcbc', '#3939db', '#e5b7e4'];
...
function shuffle(array) {
    ...
    // 배열의 마지막 요소부터 시작해서 첫 번째 요소까지 역순으로 반복
    // i는 11부터 0까지 반복
    for (let i = array.lenght -1; i > 0; i--){

        // Math.random() * (i + 1) (0부터 1까지 = i + 1해서 무작위 인덱스 생성)
        // 0 부터 i까지 무작위 인덱스 생성
