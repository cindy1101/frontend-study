...
// : ThemeProvider를 사용하여 원하고자하는 styled-components의 맥락에 전역 테마를 설정
// -> 해당 테마를 가지는 모든 자식 컴포넌트에서 접근 가능하다
...
interface ButtonProps {
    // 객체에서 속성값을 가져올 때, 해당 globalTheme.colors 객체의 키(key)값들 중에 하나를 나타냄
...
const Button = styled.button<ButtonProps> `
    /* || : true가 하나라도 true라면 실행한다. */
    background-color: ${props => props.theme.colors[props.color] || '#000'};
    color: #fff;
    padding: 8px 16px;
    border: none;
    border-radius: 4px;
    font-size: ${props => props.theme.fonts.large}; // 고정되어 있는 폰트 라지
    cursor: pointer;
    ...
    &:hover{
        background-color: ${props => {
            ...
            // -> CSS-in-JS 기법에서 색상을 조정하고 조작하는 삼수를 제공 (+ 단위 기능도 포함)
            // -> darken(), lighten() : 첫 번째 인자 alpha(투명도), 두번 째 인자값은 색상값
            // : npm i polished
...
export default function Style04() {
  ...
  return (
    ...
    <ThemeProvider theme={dynamicTheme}>
        <StateButton themeMode = {themeMode} onClick={() => setThemeMode(themeMode === 'light' ? 'dark' : 'light')}>상태 활용 버튼 (테마토글)</StateButton>
    </ThemeProvider>
    </>
  )
}
