...
/*
    ...
    - DiaryEntry 컴포넌트
    : Props로 date, content, mood 받음 -> 일기의 날짜, 내용, 기분을 보여주는 컴포넌트

    - DiaryList 컴포넌트
    : 여러 개의 DiaryEntry 컴포넌트를 렌더링
    : 전체 일기 목록과 현재 필터링 된 기분 상태를 props로 받음
    : 선택된 기분 상태에 따라 일기를 필터링하여 표시
    -> filter와 map을 사용하여 일기 목록을 처리

    - App 컴포넌트 (Review01 폴더가 대신 한다)
    : 전체 프로젝트의 루트 컴포넌트
    : 일기 목록과 현재 선택된 기분 필터 상태를 관리 (-> useState 학습 후 진행 예정)
...
// & DiaryList
// : 해당 컴포넌트가 React의 기능 컴포넌트 임을 명시, 다이어리 리스트 안에 배열을 가지고 올 거임
const DiaryList: React.FC<{entries: DiaryEntryProps[]}> = ({entries}) => {
    // {entries}: DiaryEntryProps[] -> 다이어리 안에 모든 내용
    return(
        <div>
            {/* 배열을 렌더링할 때 각각의 고유값을 전달 */}
