...
export default function State03() {
    ...
    // ! onSubmit 함수 -> 폼 제출 함수
    // : 기본 동작을 중지, 입력 데이터를 콘솔에 출력
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault(); // 폼의 기본 제출 동작 중지

        console.log('폼 데이터가 제출되었습니다: ', inputs); // 객체 타입으로 나타남
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // : 여러 개의 input을 하나의 이벤트 핸들로러 처리하기 위해서는 해당 필드에 name과 value의 속성이 반드시 존재해야 함

        // ? 비구조화 할당
        const {name, value} = e.target;
        // -> name을 key값으로 value를 속성값으로 Input 타입을 가지는 객체의 값 수정이 가능하다.

        // ? 값 수정을 위한 set 설정 함수 사용
        setInputs({

            // 기존 상태를 복사 -> input type
            // ? 스프레드 연산자 (...연산자)
            ...inputs, 
            ...
            // ? name 키를 가진 값을 value로 설정 (변경된 필드의 값만을 업데이트)
            [name]: value
        });
    }
