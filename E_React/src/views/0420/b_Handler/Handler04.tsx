...
// : 이벤트 핸들러 첨부 시 해당 이벤트를 사용할 수 있는 적절한 요소 사용이 필요하다
function Button1({onClg, onAlert, children}: ButtonProps) {
    return(
        // <div onClick={onClg} style={{border: '1px solid black', borderRadius: '5px', backgroundColor: '#eee', padding: '5px'}}>
        //     {children}
        // </div>

        <button onClick={onClg} style={{border: 'none', backgroundColor: 'white', color: 'red'}}>
            {children}
        </button>
    );
}
