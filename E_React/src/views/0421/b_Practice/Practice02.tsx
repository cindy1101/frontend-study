...
export default function Practice02() {
  ...
  const [newCandidateName, setNewCandidateName] = useState<string>(''); // -> 빈 문자열은 불리언 값으로 false
  ...
  // : 입력된 이름이 공백이 아니고 (trim) 중복되지 않을 경우 추가
  const addCandidate = () => {
    // some() 함수 (callback 함수)
  ...
  // : 선택된 후보자의 득표수를 1 증가
  const vote = () => {
      setCandidates(candidates.map(candidate => candidate.name === selectedCandidate ? {...candidate, votes: candidate.votes + 1} : candidate));
  }

  // ? 선택된 후보자를 목록에서 삭제하는 함수
  const deleteCandidate = () => {
    // 전체 후보자 목록에서 선택된 후보자의 이름과 순회되는 값의 이름이 다를 경우 새로운 배열로 반환
    // 갯수 차이 -> filter
  ...
  return(
    <>
      <div style={styles.container}>
        ...
        {/* 후보자 추가 입력과 버튼을 포함하는 영역 */}
        <div>
            <input type="text" value={newCandidateName} onChange={(e) => setNewCandidateName(e.target.value)} placeholder='새 후보자의 이름을 입력해주세요.'
            style={{width: '300px', height: '40px', margin: '10px'}}/>
            <button onClick={addCandidate}>후보자 추가</button>
        </div>

        {/* 후보자 선택 드롭 다운과 투표, 삭제 버튼을 포함하는 div */}
        <div>
          <select value={selectedCandidate} onChange={(e) => setSelectedCandidate(e.target.value)} style={{width: '310px', height: '40px', margin: '10px'}}>
              ...
              {/* 배열을 순회해서 배열 안 요소의 name 값을 가져온다 -> map 사용 (JS 문법 사용) */}
              {candidates.map((candidate, index) => (
                // 후보자 이름을 선택지로 순회 -> 후보자의 수 만큼
                <option key={index} value={candidate.name}>{candidate.name}</option>
              ))}
          </select>
          {/* 선택된 후보자가 없으면 비활성화되는 투표 버튼 */}
          {/* 만약 선택된 후보자가 없는 경우에는 selectedCandidate 변수에 초기값의 지정이 안 되어 있다. 즉, 빈 문자열이다.
              : 해당 변수가 null 또는 undefined가 되지 않음을 명시적으로 전달한다. (변수명 앞 !)
          */}
