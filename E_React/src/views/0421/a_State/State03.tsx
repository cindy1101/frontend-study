...
export default function State03() {
    ...
    // ? 1. 숫자형 (number)
    const [count, setCount] = useState<number>(0); // -> 구조 안에 요소값만 바꾼다?

    // let countVar = 0; -> countVar에 대한 값을 변경
    // countVar = 1;
    ...
    return (
        <div style={{margin: '20px', border: '1px solid black'}}>
            ...
            {/* 논리형 : 토글 버튼 */}
            <p>Visiavle : { isVisiable ? 'yes' : 'no' }</p>
            <button onClick={() => setIsVisiagle(!isVisiable)}>토글 버튼</button>

            {/* 객체형 : 사용자 정보 수정 입력 필드 */}
            <p>User : {JSON.stringify(user)}</p>
            <input type="text" name='name' value={user.name} onChange={handleUserChange} placeholder='user name'/>
