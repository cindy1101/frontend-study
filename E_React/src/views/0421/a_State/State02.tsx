...
export default function State02() {
  ...
  const validateEmail = (email: string): boolean => {
      // ? test() 메서드
      // : 주어진 문자열이 정규 표현식을 만족하는지 판별, 그 여부를 boolean으로 반환
      // : 만족할 경우 true, 만족하지 않을 경우 false
      return /\S+@\S+\.\S+/.test(email);

      // ? 정규 표현식 (예시 이미지 : devgiants75@naver.com)
      // # \S+@\S+\.\S+
      // \S+ : 하나 이상의 공백이 아닌 문자에 일치(포함) (devgiants75)
      // @ : 리터럴 문자인 @ 문자를 포함 (@)
      // \S+ : 하나 이상의 공백이 아닌 문자에 일치 (도메인 이름의 시작) (naver)
      // \. : 리터럴 점(.) 문자를 포함 (도메인의 일부)
      // \S+ : 하나 이상의 공백이 아닌 문자에 일치 (도메인의 최상위 도메인: com, net, co.kr 등)
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
      ...
      // ? 아이디 유효성 검사 (비워져 있으면 안 되게끔 구현)
      // 비구조화 할당 사용해서 이렇게 사용, 아닐 때는 !formData.id 로 작성해야지 된다
      if(!id){
        // 아이디 필드가 비어졌으면
        tempErrors.id = '아이디를 입력해주세요.'; // 오류 메시지 설정
        isValid = false; // 유효성의 상태를 false 로 설정
      }

      // ? 비밀번호 유효성 검사 (비워져 있으면 안 되게끔 구현)
      if(!password){
