...
export default function Todo() {
  ...
  // : 필요할 때만 재생성되도록 최적화
  // : 현재 상태는 정적 함수이기 때문에 컴포넌트 렌더링 시에만 생성된다.
  ...
  // : useMemo를 사용하여 필터링 결과를 메모화(캐싱)
   // -> 필터링 결과가 저장되도록, 다른 부분이 바뀌어도 필터링 되는 부분이 안 바뀌면 유지가 되도록
  ...
  //! 입력 필드에서 엔터키를 눌렀을 때, 호출되는 이벤트 핸들러
  // : 키보드 입력에 대한 이벤트 리스너
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter') {
        // 엔터 키를 클릭했다면 event target (input 창)
      ...
      <ul className='todo-list'>
        {filteredTodos.map(todo => (
          <li key={todo.id} className='todo-item'>
            {/* todo.text -> 전체 내용, 완료 여부 만 completed class 추가 */}
            <span
