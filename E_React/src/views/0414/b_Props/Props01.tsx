...
/*
    ! Props (properties, 속성)
    : 제공해서 하위 요소들에게 정보를 전달 (객체, 배열, 함수 등 JS에 포함한 모든 것들을 전달)

    ? 단일 Props 전달
    : 부모 컴포넌트로부터 자식 컴포넌트로 데이터를 전달할 때 사용
    : Props의 경우 자식 컴포넌트에서 읽기 전용 사용 (변경 불가)
...
const Wrapper: React.FC<ChildrenRtype> = ({children}) => {
    const style = {
    ...
    return (
        <div style={style}>
            {/* 
                React의 Node 요소를 포함 
                : 단일 태그 컴포넌트로 사용할 수 없다.
                : 열리고 닫히는 태그 사이의 내용으로 Node 요소(컴포넌트)가 포함되어 있어야 한다.
             */}
            {children}
        </div>
    )
...
export default function Props01() {
  return (
    ...
    <Wrapper>  
        {/* 자식 컴포넌트로 props 전달 시 호출 컴포넌트에 속성값으로 전달 */}
        <Data name='이승아' /> 
        <Hello color='pink' name='이도경' />
        <Hello color='pink' />
    </Wrapper>
  )
}
...
// <Hello color='pink' name='이도경' /> => name 없으면 오류 남 (defaultProps 전에)