...
export default function Reducer02() {
  ...
  return (
    <div>
      <h3>{state.player.name}의 Level은 {state.player.level}</h3>
      <p>Potions: {state.player.item.potions}</p>
      <button onClick={() => dispatch({type: 'LEVEL_UP'})}>Level Up</button>
      {/* 0일 경우 버튼의 기능을 상실 */}
      <button onClick={() => dispatch({type: 'USE_POTION'})} disabled={state.player.item.potions === 0}>Use Potion ({state.player.item.potions})</button>
    </div>
  )
